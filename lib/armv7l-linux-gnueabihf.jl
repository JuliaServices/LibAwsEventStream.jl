...
function Base.getproperty(x::Ptr{aws_event_stream_streaming_decoder}, f::Symbol)
    ...
    f === :prelude && return Ptr{aws_event_stream_message_prelude}(x + 184)
    f === :state && return Ptr{Ptr{aws_event_stream_process_state_fn}}(x + 196)
    f === :on_payload && return Ptr{Ptr{aws_event_stream_process_on_payload_segment_fn}}(x + 200)
    f === :on_prelude && return Ptr{Ptr{aws_event_stream_prelude_received_fn}}(x + 204)
    f === :on_header && return Ptr{Ptr{aws_event_stream_header_received_fn}}(x + 208)
    f === :on_complete && return Ptr{Ptr{aws_event_stream_on_complete_fn}}(x + 212)
    f === :on_error && return Ptr{Ptr{aws_event_stream_on_error_fn}}(x + 216)
    f === :user_context && return Ptr{Ptr{Cvoid}}(x + 220)
    return getfield(x, f)
end
